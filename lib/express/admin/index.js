...
const CUSTOM_PERSONAS_FILE = process.env.VERCEL
  ? '/tmp/custom-personas.json'
  : path.resolve(__dirname, '../../../static/myinfo/custom-personas.json')
...
router.post('/personas', (req, res) => {
  try {
    ...
    const newPersona = {
      ...
      childrenbirthrecords: Array.from({ length: noOfChildren || 0 }).map(
        (_, i) => ({
